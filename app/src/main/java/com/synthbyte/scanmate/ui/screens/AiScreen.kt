        ...
        LazyColumn(
            ...
            item {
                AiSuggestionChips(
                    onSuggestion = { suggestion ->
                        prompt = if (prompt.isBlank()) suggestion else prompt.trimEnd() + "\n\n" + suggestion
                    }
                )
            }
            ...
            item {
                OutlinedTextField(
                    ...
                    label = { Text("Paste OCR text or ask about a document") },
                    supportingText = {
                        Text(
                            if (canUseOnlineAi) "Using ${selectedModel.displayName}. Your prompt stays editable before every run."
                            else "Local document actions are ready. Add a Gemini key in Settings for deeper AI."
                        )
                    },
            ...
            item {
                Text("Suggested document actions", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold)
            }
...
@Composable
private fun AiSuggestionChips(onSuggestion: (String) -> Unit) {
    val suggestions = listOf(
        "Clean this OCR text and fix obvious recognition mistakes.",
        "Summarize this document into clear study notes.",
        "Extract names, dates, totals and important action items.",
        "Turn this into flashcards for quick revision."
    )
    Row(
    ...
    Card(
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                Text("AI document studio", color = Color.White, fontWeight = FontWeight.Bold, style = MaterialTheme.typography.titleLarge)
                Text(
                    if (canUseOnlineAi) "$selectedModel ready for summaries, cleanup and extraction"
                    else if (isOnline) "Local cleanup is ready · add a key for Gemini reasoning"
                    else "Local cleanup remains available while you are offline",
